...
class TCPSender {
  private:
    ...
    uint64_t _recv_ackno{0};                         //~ already ack number
    std::queue<TCPSegment> _segments_outstanding{};  //~ outstanding segments
    bool _syn{}, _fin{};                             //~ is SYN or FIN
    size_t _win_size{};                              //~ window size
    size_t _byte_in_flight{};                        //~ the seqNnmber of send but not ACK
    unsigned int _RTO{};                             //~ retransmission timeout
    bool _time_run{false};                           //~ is timer run
